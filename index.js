...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `bem vindo fdp @${num.split('@')[0]}\n ja vai logo mamando o reizin*${mdata.subject}*`
				...
				teks = ` vai pelo caminhão de lixo @${num.split('@')[0]} pq lixo no sol fica fedendo`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: 'TÔ PROCESSANDO AQ, VAI NO XXX POR ENQUANTO.',
				success: 'ai viado teu gif',
				error: {
					stick: ' erro ao converter saporra',
					Iv: 'ta errado esse comando ae em'
				},
				only: {
					...
					Badmin: 'Este comando só pode ser usado quando o bot se torna admin!'
}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["5521976394573@s.whatsapp.net"] // replace this with your number
			...
			switch(command) {
				...
				case 'info':
					...
					teks = `*Nome do bot* : ${me.name}\n*Número do bot* : @${me.jid.split('@')[0]}\n*Prefix* : ${prefix}\n*Contatos bloqueado* : ${blocked.length}\n*O bot ativi desde.* : ${kyun(uptime)}`
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await removeBackgroundFromImageFile({path: media, apiKey: keyrmbg, size: 'auto', type: 'auto', ranp}).then(res => {
							...
							fs.writeFileSync(ranp, buffer, (err) => {
								if (err) return reply('Falha, ocorreu um erro, tente novamente por um tempoi.')
							})
				...
				case 'tiktokstalk':
					try {
						if (args.length < 1) return client.sendMessage(from, 'Onde está o nome de usuário, em?', text, {quoted: mek})
				...
				case 'url2img':
					...
					if (args.length < 1) return reply('Qual é o tipo em')
					if (!tipelist.includes(args[0])) return reply('Desktop|tablet|tipo móvel')
					if (args.length < 2) return reply('Onde está o url em?')
				...
				case 'tstiker':
				case 'tsticker':
					if (args.length < 1) return reply('Onde está o texto, em?')
					ranp = getRandom('.png')
					rano = getRandom('.webp')
				...
				case 'bc':
					...
					if (mentioned.length > 1) {
						teks = 'esse mamou o reizin pra crlh pra ser adm\n'
				...
				case 'demote':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Rebaixado não mamou mais os adms\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
				...
				case 'kick':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('marque quem vc quer expulsar!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
				...
				case 'toimg':
					...
					break
				case 'simi':
					if (args.length < 1) return reply('Cadê o texto, em?')
					teks = body.slice(5)
